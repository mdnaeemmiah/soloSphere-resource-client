...
const BidRequests = () => {
  ...
  const getData = async () => {
    const { data } = await axiosSecure(`http://localhost:9000/bid-requests/${user?.email}`)
    return data
  }
const {mutateAsync }= useMutation({
  mutationFn: async ({id,status})=>{
    const { data } = await axiosSecure.patch(`http://localhost:9000/bid/${id}`,{ status })
      console.log(data);
      return data
  },
  onSuccess:()=>{
                  ...
                  {bids.map(bid => <tr key={bid._id}>
                    ...
                    <td className='px-4 py-4 text-sm font-medium text-gray-700 whitespace-nowrap'>
                      <div className={`inline-flex items-center px-3 py-1 rounded-full gap-x-2 bg-yellow-100/60 text-yellow-500
                                                  ${bid.status === 'Pending' &&
                        'bg-yellow-100/60 text-yellow-100'
